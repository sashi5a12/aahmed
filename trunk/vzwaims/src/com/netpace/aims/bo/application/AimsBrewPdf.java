...
public class AimsBrewPdf {
	static Logger log = Logger.getLogger(AimsBrewPdf.class.getName());
	...
	public static void getneratePdf(HashMap appMap, OutputStream out) throws Exception{
		log.debug("AimBrewPdf.getneratePdf Start:");
		Document doc = null;
//		PdfWriter writer = null;
		try {
			doc = new Document(PageSize.LETTER);
//			writer = PdfWriter.getInstance(doc, new FileOutputStream("aims.pdf"));
...
//			style.loadTagStyle("th","", "");
			...
			AimsBrewApp aimsBrewApp=(AimsBrewApp) appMap.get("AimsBrewApp");
			AimsBrewFiles aimsBrewFiles=(AimsBrewFiles) appMap.get("AimsBrewFiles");
			AimsBrewAppClob aimsBrewClobs=(AimsBrewAppClob) appMap.get("AimsBrewClobs");
			...
			if (aimsApp != null){
				...
				if (aimsAppFiles != null){
					if (aimsBrewFiles.getCompanyLogo() != null){
						companyLogo=aimsBrewFiles.getCompanyLogo().getBytes(1, (int)aimsBrewFiles.getCompanyLogo().length());
					}
					if (aimsBrewFiles.getTitleShot() != null){
						titleShot=aimsBrewFiles.getTitleShot().getBytes(1, (int)aimsBrewFiles.getTitleShot().length());
					}
				}				
			...
			AimsBrewPdf.addAgreement(doc, null, disclaimer.getDisclaimerStr());
			
			if (aimsBrewApp != null && StringUtils.isNotEmpty(aimsBrewApp.getProductDescription())){
				AimsBrewPdf.writeData(doc, "Product Description","<p>"+aimsBrewApp.getProductDescription()+"</p>");
			}
			...
			if (aimsAppFiles != null){
				...
				if (callFunction){
					AimsBrewPdf.addScreenShots(doc, "Screenshots ", s1, s2, s3, s4, s5);
				}
			}
			
			if (aimsBrewClobs != null){
				if(aimsBrewClobs.getUsingApplication() != null){
					Clob clob = aimsBrewClobs.getUsingApplication();
					String str = clob.getSubString(1, (int)clob.length());
					if (StringUtils.isNotEmpty(str)){
						AimsBrewPdf.writeData(doc, "Using the Application", str);
					}
				}
				if(aimsBrewClobs.getTipsAndTricks() != null){
					Clob clob = aimsBrewClobs.getTipsAndTricks();
					String str = clob.getSubString(1, (int)clob.length());
					if (StringUtils.isNotEmpty(str)){
						AimsBrewPdf.writeData(doc, "Tips and Tricks", str);
					}
				}
				if(aimsBrewClobs.getFaq() != null){
					Clob clob = aimsBrewClobs.getFaq();
					String str = clob.getSubString(1, (int)clob.length());
					if (StringUtils.isNotEmpty(str)){
						AimsBrewPdf.writeData(doc, "FAQ", str);
					}
				}
				if(aimsBrewClobs.getTroubleshooting() != null){
					Clob clob = aimsBrewClobs.getTroubleshooting();
					String str = clob.getSubString(1, (int)clob.length());
					if (StringUtils.isNotEmpty(str)){
						AimsBrewPdf.writeData(doc, "Troubleshooting", str);
					}
				}
				if(aimsBrewClobs.getDevelopmentCompanyDisclaimer() != null){
					Clob clob = aimsBrewClobs.getDevelopmentCompanyDisclaimer();
					String str = clob.getSubString(1, (int)clob.length());
					if (StringUtils.isNotEmpty(str)){
						AimsBrewPdf.writeData(doc, "Development Company Disclaimer", str);
					}
				}
				if(aimsBrewClobs.getAdditionalInformation() != null){
					Clob clob = aimsBrewClobs.getAdditionalInformation();
					String str = clob.getSubString(1, (int)clob.length());
					if (StringUtils.isNotEmpty(str)){
						AimsBrewPdf.writeData(doc, "Additional Information", str);
					}
				}				
			}			
		} catch (Exception e) {
		...
		log.debug("AimBrewPdf.getneratePdf End:");
	}
	...
	public static void main(String args[]){
		...
		try {
			...
			AimsBrewPdf.getneratePdf(appMap, os);
		} catch (Exception e) {
			log.error(e,e);
			e.printStackTrace();
		} finally {
