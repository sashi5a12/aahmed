 ...
 * Application.
 * 
 * @struts.action path="/javaOffDeckApplicationSetup"
 ...
 * @author Waseem Akram
 */
public class JavaOffDeckApplicationSetupAction extends BaseAction
{
	...
	{
	    // the GTM system checks whether the user has a valid accepted contract for the platform,
		...
		// if there is a Click Through Contract for that platform in the system
		if (numOfJavaCon > 0)
		{
		...
		// if there is NO Click Through Contract for that platform in the system
		{
	...
	javaAppUpdForm.setRingNumber(AimsConstants.CONTRACT_RING_3_ID);
	...
	javaAppUpdForm.setEnterpriseApp(AimsConstants.NO_CHAR);
	javaAppUpdForm.setContentType(AimsConstants.JAVA_CONTENT_TYPE_APPLICATION);
	
	javaAppUpdForm.setAppKeyword(JavaUtils.getAppKeyword());
