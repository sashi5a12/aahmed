...
public class TypeASelectorImpl extends AbstractSelector {

	private static Logger log = Logger.getLogger(TypeASelectorImpl.class
			.getName());

	public TypeASelectorImpl() {
		super(new FeedType('A', "First Contract Accepted -- New Alliance",
				"First Contract Accepted -- New Alliance"));
	}

	public Collection<DeveloperFeed> execute(Connection conn, String startTime,
			String endTime) throws DBIssueException {
		Collection<DeveloperFeed> atypeAlliancesFromAudit = getTypeARecrods(
				conn, startTime, endTime);
		return atypeAlliancesFromAudit;
	}

	private Collection<DeveloperFeed> getTypeARecrods(Connection conn,
			String startTime, String endTime) throws DBIssueException {
		log.debug("Fetching Type A records!!!");
		...
		try {
			...
			StringBuffer typeAQuery = new StringBuffer(
					"select "
							+ "distinct a_contract.alliance_id, a_contract.contract_id, "
							+ "a_contract.accept_decline_date,  alliance.vendor_id, "
							+ "a_contract.aud_action, contracts.contract_ref_id "

							+ "from aims_alliance_contracts$aud a_contract, aims_contracts$aud contracts, aims_alliances$aud "
							+ "alliance  "
							...
							+ "and a_contract.accept_decline_date < TO_DATE(?, 'MM/DD/YYYY HH24:MI:SS') "
							+ ") "
							+ "and (a_contract.alliance_id,accept_decline_date) in (select a_contract.alliance_id,min "
							+ "(a_contract.accept_decline_date) accept_decline_date "
							+ "from aims_alliance_contracts$aud a_contract, aims_contracts$aud contracts, aims_alliances$aud "
							+ "alliance  "
							+ "where contracts.contract_id = a_contract.contract_id "
							+ "and alliance.alliance_id = a_contract.alliance_id "
			...
			log.info("Type A Query to be executed::\n" + typeAQuery);
			...
			while (rSet.next()) {
				...
				Timestamp acceptance_time = rSet.getTimestamp(3);
			...
			log.debug("A Total of " + feeds.size() + " Type A records found.");
			log.debug("Type A records fetching done!!!");
			return feeds;
