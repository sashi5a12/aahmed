...
Copyright (c) 2002-2007 Netpace Systems. All rights reserved.
Created 1.12.2002 by Walter Zorn (Web: http://www.netpace.com )
...
config. CloseBtnColors	= ['#e6e4e3', '#000000', '#e6e4e3', '#000000']	  // [Background, text, hovered background, hovered text] - use empty strings '' to inherit title colors
...
config. TitleBgColor	= '#666666'		// If empty string '', BorderColor will be used
...
function tt_Tip(arg, t2t)
{
	...
	arg[0] = replaceAll(arg[0],"\r\n","<BR/>");
	arg[0] = replaceAll(arg[0],"\r","<BR/>");
	arg[0] = replaceAll(arg[0],"\n","<BR/>");
	
	//Entry is journal.	
	var je=arg[0].split("~~!~~");
	var longEntry=arg[0].split("<hr/>");
	var longBr=arg[0].split("<br/>");
	if (je.length == 2 && longEntry.length>=6){
	...
	else if (longBr!=null && longBr.length>=25){
		config.ScrollBar=true;
		config.CloseBtn=true;
		config.ClickClose=true;
		config.Sticky=true;
...
function tt_MkTipSubDivs()
{
	...
	tt_aElt[0].innerHTML =
		...
		+ '<div id="WzBoDy" style="position:relative;z-index:0;'
	        + (config.ScrollBar? 'height:'+(screen.height/2)+'px;' : '')
		+ 'overflow:auto;">'
...
function tt_FixSize(iOffY, iOffSh)
{
	...
	/* Added By Imtiaz 
	if( parseInt(tt_aElt[5].style.width.replace("px","")) > winW )
		tt_aElt[5].style.width = (winW / 2) + "px";
	 Added By Imtiaz */
	// Title
	if(tt_aElt[1])
	{
		...
		/* Added By Imtiaz 
		if( parseInt(tt_aElt[1].style.width.replace("px","")) > winW )
			tt_aElt[1].style.width = (winW / 2) + "px";
		 Added By Imtiaz */
		/* Added By Imtiaz 
		if( parseInt(tt_aElt[2].style.width.replace("px","")) > winW )
			tt_aElt[2].style.width = (winW / 2) + "px";
		 Added By Imtiaz */
	}
	...
	if(tt_aElt[i])
	{
		...
		/* Added By Imtiaz 
		if( parseInt(tt_aElt[i].style.width.replace("px","")) > winW )
			tt_aElt[i].style.width = (winW / 2) + "px";
		 Added By Imtiaz */
...
function tt_Show()
{
	...
	/* Added By Imtiaz - screen overflow problem */
	if( parseInt(css.width.replace("px","")) > winW )
		css.width = (winW / 2) + "px";
	tt_iState |= 0x2;
	if(tt_aV[FADEIN])
