CREATE OR REPLACE PACKAGE BODY BDS_PUBLIC_SEARCH_PAGE
IS
...
SELECT distinct
            ...
            FROM
                 ...
                 aims_apps ap,
                 aims_alliances al,
                 bds_solution_market_segments bsms
            WHERE
                 e.enterprise_apps_id = ap.apps_id
                 and ap.alliance_id = al.alliance_id
                 and ap.apps_id = bsms.solution_id
                 and bsms.is_public = 'Y'                 
            ORDER BY 2;
...
/* -------------------------------------------------------------------------------------------------------------  */
   ...
   PROCEDURE get_mkt_segment_cursor
         (
           p_mkt_seg_id                    IN number,              -- market segment id
           p_out_mkt_segment                   OUT TYPES.cursor_type    -- Result cursor
         )
    IS
   ...
   EXCEPTION
      WHEN OTHERS THEN
          IF(p_out_mkt_segment%ISOPEN) THEN
            CLOSE p_out_mkt_segment;
          END IF;
          RAISE;

   END get_mkt_segment_cursor;
...
/* -------------------------------------------------------------------------------------------------------------  */
   ...
   BEGIN
        ...
        get_mkt_segment_cursor          (v_mkt_seg_id, p_out_mkt_segment);

   END get_search_page;

/* -------------------------------------------------------------------------------------------------------------  */

END BDS_PUBLIC_SEARCH_PAGE; -- Package Body BDS_SEARCH_PAGE
/
