...
<div class="grayBoxLayouts">
	<div class="threeColmnLayout">
		...
		<div class="columnOne">
			...
			<label class="inputlabel"><fmt:message key="productVO.video.title"/></label><br/>
			<span class="label-info-small" ><fmt:message key="productVO.appVideo.format"/></span>
			<div class="clearboth"></div>

			<div id="upload_<%=VAPConstants.PRODUCT_PRODUCT_VIDEO%>" <c:if test="${not empty productVO.product_video and not empty productVO.product_video.mediaId}">style="display:none;"</c:if>></div>
			<div class="uploadeded-File" id='uploaded_<%=VAPConstants.PRODUCT_PRODUCT_VIDEO%>' <c:if test="${empty productVO.product_video or empty productVO.product_video.mediaId}">style="display:none;"</c:if>>
				...
				<span><a 
					id="delete_<%=VAPConstants.PRODUCT_PRODUCT_VIDEO%>" 
					rel="<c:if test="${not empty productVO.product_video and not empty productVO.product_video.fileType}">${productVO.product_video.fileType}</c:if>"
					name="<c:if test="${not empty productVO.product_video and not empty productVO.product_video.fileName}">${productVO.product_video.fileName}</c:if>"
					class="<c:if test="${not empty productVO.product_video and not empty productVO.product_video.mediaId}">${productVO.product_video.mediaId}</c:if>"
					href="#" 
		...
		<div class="columnTwo">
			...
			<label class="inputlabel width100"><fmt:message key="productVO.appVideo.phone"/></label><br/>
			<span class="label-info-small" ><fmt:message key="productVO.appVideo.format"/></span>
			<div class="clearboth"></div>

			<div id="upload_<%=VAPConstants.PRODUCT_PHONE_APP_VIDEO%>" <c:if test="${not empty productVO.phone_app_video and not empty productVO.phone_app_video.mediaId}">style="display:none;"</c:if>></div>
			<div class="uploadeded-File" id='uploaded_<%=VAPConstants.PRODUCT_PHONE_APP_VIDEO%>' <c:if test="${empty productVO.phone_app_video or empty productVO.phone_app_video.mediaId}">style="display:none;"</c:if>>
				...
				<span><a 
					id="delete_<%=VAPConstants.PRODUCT_PHONE_APP_VIDEO%>" 
					rel="<c:if test="${not empty productVO.phone_app_video and not empty productVO.phone_app_video.fileType}">${productVO.phone_app_video.fileType}</c:if>"
					name="<c:if test="${not empty productVO.phone_app_video and not empty productVO.phone_app_video.fileName}">${productVO.phone_app_video.fileName}</c:if>"
					class="<c:if test="${not empty productVO.phone_app_video and not empty productVO.phone_app_video.mediaId}">${productVO.phone_app_video.mediaId}</c:if>"
					href="#" 
					title="Delete" ><tg:img src="/images/uploadedFile-icon.png" width="16" height="16" alt="Delete" border="0" /></a></span>
				<div class="clearboth"></div>
		...
		<div class="columnThree">
			<h1>&nbsp;</h1>
			<label class="inputlabel width100"><fmt:message key="productVO.appVideo.tablet"/></label><br/>
			<span class="label-info-small" ><fmt:message key="productVO.appVideo.format"/></span>
			<div class="clearboth"></div>

			<div id="upload_<%=VAPConstants.PRODUCT_TABLET_APP_VIDEO%>" <c:if test="${not empty productVO.tablet_app_video and not empty productVO.tablet_app_video.mediaId}">style="display:none;"</c:if>></div>
			<div class="uploadeded-File" id='uploaded_<%=VAPConstants.PRODUCT_TABLET_APP_VIDEO%>' <c:if test="${empty productVO.tablet_app_video or empty productVO.tablet_app_video.mediaId}">style="display:none;"</c:if>>
				...
				<span><a 
					id="delete_<%=VAPConstants.PRODUCT_TABLET_APP_VIDEO%>" 
					rel="<c:if test="${not empty productVO.tablet_app_video and not empty productVO.tablet_app_video.fileType}">${productVO.tablet_app_video.fileType}</c:if>"
					name="<c:if test="${not empty productVO.tablet_app_video and not empty productVO.tablet_app_video.fileName}">${productVO.tablet_app_video.fileName}</c:if>"
					class="<c:if test="${not empty productVO.tablet_app_video and not empty productVO.tablet_app_video.mediaId}">${productVO.tablet_app_video.mediaId}</c:if>"
					href="#" 
					title="Delete" ><tg:img src="/images/uploadedFile-icon.png" width="16" height="16" alt="Delete" border="0" /></a></span>
				<div class="clearboth"></div>
