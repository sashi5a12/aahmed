...
public class DBErrorFinder
{
		...
		public static boolean searchUniqueConstraintErrors(String exMessage, String[] keyList, VzdnException aimsException)
    {
			...
			for (int i=0;i<keyList.length/2;i++)
			{
				...
				if (exMessage.indexOf(VzdnConstants.UNIQUE_CONSTRAINT_MESSAGE.format(objs)) > -1) 
				{		
	...
	public static boolean searchUniqueConstraintErrorsWithSchemaName(String exMessage, String[] keyList, VzdnException aimsException)
    {
			boolean errorFound = false;
            ...
            for (int i=0;i<keyList.length/2;i++)
			{
                ...
                if (exMessage.indexOf(VzdnConstants.UNIQUE_CONSTRAINT_MESSAGE_DB.format(objs)) > -1)
				{
					aimsException.addException(new UniqueConstraintException(keyList[(i*2)+1]));
					errorFound=true;
                }
    ...
    public static boolean searchIntegrityConstraintErrors(String exMessage, String[] keyList, VzdnException aimsException)
    {
			boolean errorFound = false;
			...
			for (int i=0;i<keyList.length/2;i++)
			{
				...
				if (exMessage.indexOf(VzdnConstants.INTEGRITY_CONSTRAINT_MESSAGE .format(objs)) > -1) 
				{		
    ...
    public static boolean isIntegrityConstraintError(String exMessage, VzdnException aimsException)
    {
			boolean errorFound = false;
			
			if (exMessage.indexOf(VzdnConstants.INTEGRITY_CONSTRAINT_MESSAGE_PREFIX) > -1) 
			{		
				aimsException.addException(new IntegrityConstraintException(VzdnConstants.INTEGRITY_CONSTRAINT_RES_MESSAGE));
				errorFound=true;
			} 			
      return errorFound;
    }
    ...
    public static ActionErrors populateActionErrors(VzdnException aimsExceptionList)
    {
			VzdnException aimsException = null;
			ActionErrors errors = new ActionErrors();
			for	(Iterator it = aimsExceptionList.getCollection().iterator();	it.hasNext();) 
			{
				aimsException = (VzdnException) it.next();			
