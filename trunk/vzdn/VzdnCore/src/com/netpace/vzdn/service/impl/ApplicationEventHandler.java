...
public class ApplicationEventHandler extends BaseEventHandler
{
    public void handleEvent(VzdnEventObject eventObj)
    {
        ...
        try
        {
            ...
            System.out.println("\n\n\n ApplicationId : " + eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_APPS_ID));
            System.out.println("\n\n\n AllianceId : " + eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_ALLIANCE_ID));
             ...
             *
             */
            System.out.println("\n\n\n UserIds : " + eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_USER_IDS));
            System.out.println("\n\n\n Adhoc Emails : " + eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_ADHOC_EMAILS));
            System.out.println("\n\n\n EventId : " + ((VzdnEventLite) eventObj.getEvent()).getEventId().longValue());

            if (eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_ALLIANCE_ID) != null)
                statement.setLong(1, (new Long(eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_ALLIANCE_ID).toString())).longValue());
            else
                statement.setLong(1, 0);

            if (eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_APPS_ID) != null)
                statement.setLong(2, (new Long(eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_APPS_ID).toString())).longValue());
            else
                statement.setLong(2, 0);

            statement.setLong(3, ((VzdnEventLite) eventObj.getEvent()).getEventId().longValue());
            statement.setString(4, eventObj.getPropertiesWithPattern().toString());

            if (eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_USER_IDS) != null)
                statement.setString(5, eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_USER_IDS).toString());
            else
            ...
            while (results.next())
            {
                ...
                for (int j = 0; embeddedResults.next(); j++)
                {
                    ...
                    System.out.println("AimsNotificationConstants.NOTIFICATION_EMAIL_TYPE : " + VzdnNotificationConstants.NOTIFICATION_EMAIL_TYPE);
                    if (VzdnNotificationConstants.NOTIFICATION_EMAIL_TYPE.equalsIgnoreCase(embeddedResults.getString(2)))
                    {
                        ...
                        try
                        {
                            ...
                            statement2.setString(4, VzdnNotificationConstants.FROM_ADDRESS_ID);
                ...
                if (eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_ADHOC_EMAILS) != null)
                {

                    String[] strArray = StringFuncs.tokenize(eventObjProperties.get(VzdnNotificationConstants.HANDLER_PROPERTY_ADHOC_EMAILS).toString(), ",");
                    ...
                    for (int i = 0; i < strArray.length; i++)
                    {
                        ...
                        try
                        {
                            MailUtils.sendMail(strArray[i], VzdnNotificationConstants.FROM_ADDRESS, message_title, null, message_body, attachments);
                        }
                        catch (SendFailedException sfe)
                        {
                            System.out.println("SendFailedException occured in ApplicationEventHandler ");
    ...
    public CallableStatement fillDummyStatement(){
    	return null;
    }
}
