...
<h:panelGrid columns="2" cellpadding="2" cellpadding="2">
	<h:panelGroup rendered="#{DialogManager.bean.workflowListSize != 0}">
			<h:panelGrid id="workflow-heading">
				<h:outputText value="" />
			</h:panelGrid>
	</h:panelGroup>
</h:panelGrid>
...
<h:panelGrid id="netpace" columns="1" cellpadding="2" cellpadding="2" width="100%" style="display:none" >
   <h:panelGroup rendered="#{DialogManager.bean.workflowListSize != 0}">
      <h:outputText value="#{msg.submit_workflow_selection}" />
      <h:panelGrid columns="2" cellpadding="2" cellpadding="2">
         <a:selectList var="rr" id="workflow-list" multiSelect="false" styleClass="noBrColumn" itemStyle="padding-top: 3px;"
...
function selectWorkFlow(){
	...
	try{
	 var str = document.getElementById('dialog:dialog-body:col12-path').innerHTML;
	 if(str.indexOf("devCenters",0)==-1){
		 var array1 = document.getElementsByTagName('input');
		 for(i=0; i < array1.length ; i++){
			if(array1[i].type=='radio' && array1[i].type.length>0){
				if(array1[i].value=='jbpm$scwf:Default')
				array1[i].checked=true;
			}
		}
		 document.getElementById('dialog:dialog-body:workflow-heading').innerHTML='Default';
	}else{
	     str1 =  str.substring(str.indexOf("devCenters",0),str.length);
		 str2 =  str1.substring(str1.indexOf("/",0)+1,str1.length);	
		 str3 = str2.substring(0,str2.indexOf("/"));
		 if(str3=='Brew' || str3=='Java' || str3=='Dashboard' || str3=='LBS' || str3=='Messaging_PSMS' || str3=='Mobile_Web' || str3=='Smart_Phone' || str3=='Enterprise' || str3=='Network_Enablers' || str3=='JIL' || str3=='MusicVideo'){
			 document.getElementById('dialog:dialog-body:workflow-heading').innerHTML=str3;	
			 prefix= "jbpm$scwf:";
			 prefix=prefix+""+str3;
		     var array1 = document.getElementsByTagName('input');
			 for(i=0; i < array1.length ; i++){
				if(array1[i].type=='radio' && array1[i].type.length>0){
					if(array1[i].value==prefix)
					array1[i].checked=true;
				}
			}
		}else{
