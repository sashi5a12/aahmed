...
public class UserMgmtAction extends BaseAction implements Preparable{
	
	private static Logger log = Logger.getLogger(UserMgmtAction.class);
	...
	public String toggleUserType() throws VzdnSecurityException, NotLoggedInException{
		...
		try
		{
			...
			log.error("Problem in UserMgmtAction.toggleUserType:\n" + e);
			e.printStackTrace();
			return INPUT;
		}
		return SUCCESS;
	...
	public String editSave() throws VzdnSecurityException, NotLoggedInException{
		...
		try
		{
			...
			//The following bloack of code is for the sake of view-only on the page when a validation error occurs
			////////////////////////////////////////////////			
			List<Integer> userTypes = new ArrayList<Integer>();
			List<VzdnSysRoles> allAvailableRoles = new ArrayList<VzdnSysRoles>();
			userTypes.add(VzdnUserTypes.Both.getValue());
			...
			if(newuser.getUserType().getTypeId().intValue() == VzdnConstants.VerizonUser && assignedRoles.size() < 1){
				addFieldError("assignedRoles",getText("usermgmt.edituser.atleast.one.role"));
				return INPUT;
			}
			...
			getSession().setAttribute("successMessage", getText("usermgmt.edituser.successmsg"));
		} 
