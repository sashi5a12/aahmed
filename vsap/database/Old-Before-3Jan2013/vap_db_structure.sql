...
CREATE TABLE `vap_company` (
  ...
  `terms_accepted_by` INT(10) UNSIGNED DEFAULT NULL,
...
CREATE TABLE `vap_nda_vendor_data` (
  ...
  KEY `idx_vap_registration_status_vap_company` (`current_status`),
  ...
  CONSTRAINT `FK_vap_registration_status_vap_company` FOREIGN KEY (`current_status`) REFERENCES `vap_registration_status` (`registration_status_id`)
) ENGINE=INNODB DEFAULT CHARSET=utf8;
...
CREATE TABLE `vap_application_properties` (
  ...
  `updated_by` VARCHAR(50) DEFAULT NULL,
  `is_active` VARCHAR(1) DEFAULT '1',
  PRIMARY KEY `pk_property_id` (`property_id`),
  UNIQUE KEY `uk_name` (`name`)
);

ALTER TABLE vap_company ADD COLUMN main_contact INT(10) UNSIGNED UNIQUE;
ALTER TABLE vap_company ADD KEY `idx_vap_company_main_contact` (`main_contact`);
ALTER TABLE vap_company ADD FOREIGN KEY `fk_vap_company_main_contact` (`main_contact`) REFERENCES vap_user(user_id);
