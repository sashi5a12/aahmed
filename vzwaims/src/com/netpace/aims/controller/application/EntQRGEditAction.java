 ...
 *
 * @struts.action path="/entQrgEdit"                
 *                scope="request" 
 *				  name="EntAppsSpotLightForm"
 *				  validate="true"
 *                input="/application/qrgCreateUpdate.jsp"
 * @struts.action-forward name="view" path="/entAppsSpotlights.do?task=view&amp;spotLightTypeId=9" redirect="true"
 * @author Ahson Imtiaz
 */

public class EntQRGEditAction extends BaseAction
{

    static Logger log = Logger.getLogger(EntQRGEditAction.class.getName());
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
    {
        ...
        {
            Collection AimsSpotLights = EntAppsSpotlightsManager.getSpotLightRecords(enterprise_apps_id, user_type, spotLightTypeId);

            if (AimsSpotLights.size() <= 0)
            {
                try
                {
        ...
        return new ActionForward("/entAppsSpotlights.do?task=view&spotLightTypeId=9&enterpriseAppsId=" + allianceSpotLightForm.getEnterpriseAppsId(), true);
    }
}
