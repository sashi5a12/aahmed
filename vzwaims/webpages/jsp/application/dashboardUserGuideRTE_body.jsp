...
<%@page import="com.netpace.aims.util.AimsConstants"%><%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean"%>
...
<script type="text/javascript">
		...
		if (opener){
			...
			if ( fieldTextRef && editorInstance.EditMode == FCK_EDITMODE_WYSIWYG ){
				...
				editorInstance.SetData(fieldTextRef.value);
					<%--
					updateFlag=true;
					editorInstance.SetData(editorInstance.GetXHTML(editorInstance.InsertHtml(fieldTextRef.value)));
					--%>
				}
			}
		...
		if (editor && opener){
			...
			if (fieldNo){
				...
				if (text.length > 0){
					...
					if (len > <%=AimsConstants.DASHBOARD_USER_GUIDE_FIELDS_LENGTH%>){
						alert("Text cannot exceed "+ <%=AimsConstants.DASHBOARD_USER_GUIDE_FIELDS_LENGTH%>+ " characters");
						editor.ResetIsDirty();
						return;
					}
					fieldTextRef.value=text;
					fieldLenRef.value = len;
					fieldCountRef.value = <%=AimsConstants.DASHBOARD_USER_GUIDE_FIELDS_LENGTH%> - len;
				}
	...
	function setLabel(){
		...
		if(opener){
			...
			if (num){
				if (num == 1){
					document.getElementById('label').innerHTML='Using the application';
				}
				else if (num == 2){
					document.getElementById('label').innerHTML='Tips and Tricks';
				}
				else if (num == 3){
					document.getElementById('label').innerHTML='FAQ';
				}
				else if (num == 4){
					document.getElementById('label').innerHTML='Troubleshooting';
				}
				else if (num == 5){
					document.getElementById('label').innerHTML='Development Company Disclaimer';
				}
				else if (num == 6){
					document.getElementById('label').innerHTML='Additional Information';
				}
			}
			else {
