		...
		if (editor && opener){
			...
			if (fieldNo){
				...
				if (text.length > 0){
					...
					if (len > <%=AimsConstants.BREW_USER_GUIDE_FIELD_LEN%>){
						alert("Text cannot exceed "+<%=AimsConstants.BREW_USER_GUIDE_FIELD_LEN%>+" characters");
						editor.ResetIsDirty();
						return;
					}
					fieldTextRef.value=text;
					fieldLenRef.value = len;
					fieldCountRef.value = <%=AimsConstants.BREW_USER_GUIDE_FIELD_LEN%> - len;
				}
