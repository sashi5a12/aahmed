				...
				<td width="100%" >				
					<div style="float:right">
						...
						<%if (isAllianceUser) {%>
							...
							<%if (!statusSaved ) {%>
								...
								<%  if(isRFI && !isVerizonUser) { %>
									<div class="redBtn" style="float:left; margin-left:10px; margin-top:3px" id="SaveAgain" title="Resubmit">
                                    	<div>
                                    		<div>
                                    			<div onClick="document.forms[0].appSubmitType.value='<%=AimsConstants.AIMS_JAVA_RESUBMIT%>';document.forms[0].submit();">Resubmit
                                    			</div>
	                                    	</div>
    	                                </div>
        							</div>
						...
						<%if (isVerizonUser) {%>
							...
							<%if (isPage3 && showExecute) { %>
								<div class="redBtn" style="float:left; margin-left:10px; margin-top:3px" id="ExecuteAction" title="Execute Action">
										<div><div><div onClick="document.forms[0].appSubmitType.value='<%=AimsConstants.AIMS_EXECUTE_ACTION%>';submitForm();document.forms[0].submit();">Execute Action</div></div></div>
								</div>
							<%} %>
							<%-- 
